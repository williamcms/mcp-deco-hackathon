...
export function computeEconomics(
	...
	return {
		...
		bundleMarginPct: hasCost
			? round(((revenueWithCost - cost) / revenueWithCost) * 100)
			: null,
		...
		incrementalMargin:
			bundleMargin != null ? round(incrementalOrders * bundleMargin) : null,
	};
}
...
export function computeViability(
	...
	// Unidades por kit: quanto de cada produto sai quando a combinação acontece.
	const unitsInBundle = new Map<string, number>();
	for (const transaction of transactions) {
		if (!containsAll(transaction, productIds)) continue;
		for (const productId of productIds) {
			const line = transaction.lines.get(productId);
	...
	for (const productId of productIds) {
		...
		const totalUnits = unitsInBundle.get(productId) ?? 0;
		const perBundle =
			coOccurrenceOrders > 0 ? totalUnits / coOccurrenceOrders : 1;
