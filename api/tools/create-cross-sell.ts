...
import {
	commercialCampaignEvidenceSchema,
	commercialCampaignRecordSchema,
	recordCommercialCampaign,
...
export const createCrossSellInputSchema = z.object({
	...
	campaign: commercialCampaignEvidenceSchema
		.optional()
		.describe(
			"Evidências opcionais da oportunidade que originou a ação. A interface preenche este campo para registrar a campanha; não use valores inventados.",
		),
});
...
export const createCrossSellOutputSchema = z.object({
	...
	campaign: commercialCampaignRecordSchema
		.nullable()
		.describe("Campanha de acompanhamento registrada após uma gravação efetiva. Null em simulações ou quando nenhum novo item foi adicionado."),
...
export const createCrossSellTool = (env: Env) =>
	createTool({
		...
		description:
			'Ação interna, sem UI própria: grava produtos complementares (cross-sell) direto na Shopify, no metafield reservado que alimenta "Produtos complementares" no admin e os widgets de recomendação de produto ("frequently bought together" / "você também pode gostar") nos temas que leem esse campo — não cria um produto novo, diferente de create_bundle. Por padrão apenas SIMULA (dryRun = true): mostra o que já está configurado, o que seria adicionado e a lista final, sem escrever nada. Chame de novo com dryRun = false para gravar. Após uma gravação efetiva, registra uma campanha compacta no metafield da app para acompanhamento posterior. Acionada pela seleção de produtos no canvas de cross-sell de discover_combinations — não chame diretamente pelo chat sem antes ter productId e relatedProductIds de uma análise de discover_combinations. Precisa dos escopos write_products e read_products.',
		...
		execute: async ({ context }) => {
			...
			let campaign = null;
			if (merge.addedIds.length > 0) {
				try {
					campaign = await recordCommercialCampaign(credentials, {
						action: "cross_sell",
						sourceProductId: ctx.product.id,
						relatedProducts: toRefs(merge.addedIds).map((product) => ({
							id: product.id,
							title: product.title,
							handle: product.handle,
						})),
						evidence: context.campaign ?? null,
					});
				} catch (error) {
					warnings.push(
						`O cross-sell foi gravado, mas o acompanhamento não pôde ser registrado: ${error instanceof Error ? error.message : String(error)}`,
					);
				}
			}
