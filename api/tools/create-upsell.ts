		...
		execute: async ({ context }) => {
			...
			if (merge.addedIds.length > 0) {
				try {
					...
					warnings.push(
						`O upsell foi gravado, mas o acompanhamento não pôde ser registrado: ${error instanceof Error ? error.message : String(error)}`,
					);
				}
			}
