...
export const CREATE_BUNDLE_RESOURCE_URI = "ui://mcp-app/create-bundle";
...
const componentInputSchema = z.object({
	...
	quantity: z
		...
		.max(2000)
		.optional()
		.describe("Unidades deste produto dentro do kit. Padrão: 1."),
	options: z
		.array(
			z.object({
				...
				values: z
					.array(z.string())
					.min(1)
					.describe('Valores oferecidos no kit, ex: ["M", "G"].'),
			}),
		)
		.optional()
		.describe(
...
export const createBundleInputSchema = z.object({
	components: z
		...
		.max(10)
		.describe(
			"Produtos que formam o kit, normalmente os de uma combinação devolvida por discover_combinations. Mínimo 2.",
		),
	title: z
		.string()
		.min(1)
		...
		.optional()
		.describe(
			'Título do produto do kit. Padrão: "Kit A + B" com os títulos dos componentes.',
		),
	discountPercentage: z
		...
		.optional()
		.describe(
			"Desconto sobre a soma dos componentes, em %. Ignorado se fixedPrice for informado.",
		),
	fixedPrice: z
		.number()
		.min(0)
		.optional()
		.describe(
			"Preço final do kit, na moeda da loja. Tem precedência sobre discountPercentage.",
		),
	status: z
		.enum(["DRAFT", "ACTIVE"])
		.optional()
		.describe(
			"Status do produto criado. Padrão: DRAFT — o kit nasce fechado para revisão antes de ir ao ar.",
		),
	tags: z
		.array(z.string())
		.max(20)
		.optional()
		.describe('Tags do produto do kit. Padrão: ["bundle"].'),
	descriptionHtml: z
		.string()
		.max(10000)
		.optional()
		.describe("Descrição do kit em HTML, exibida na página do produto."),
	dryRun: z
		.boolean()
		.optional()
		.describe(
	...
	maxPollAttempts: z
		...
		.optional()
		.describe(
			"Tentativas de leitura do status da criação, uma por segundo. Padrão: 20.",
		),
});
...
export const createBundleOutputSchema = z.object({
	mode: z
		.enum(["simulation", "created"])
		.describe("simulation = nada foi escrito na loja; created = bundle existe"),
	shop: z.string(),
	title: z.string(),
	pricing: z.object({
		...
		marginCoverage: z
			.number()
			.describe("% da receita do kit com custo cadastrado"),
	}),
...
export const createBundleTool = (env: Env) =>
	createTool({
		id: "create_bundle",
		description:
			"Etapa 3 da descoberta: transforma uma combinação em um bundle de verdade na Shopify (productBundleCreate), com estoque derivado dos componentes. Por padrão apenas SIMULA (dryRun = true): devolve preço do kit, economia, margem, cenários de desconto e quantos kits o estoque sustenta, sem escrever nada. Chame de novo com dryRun = false para criar o produto, aplicar o preço e definir o status. Use depois de discover_combinations, passando os produtos da combinação escolhida. Precisa dos escopos write_products, read_products e read_inventory, e de uma loja com o recurso de bundles habilitado.",
		inputSchema: createBundleInputSchema,
		outputSchema: createBundleOutputSchema,
		_meta: { ui: { resourceUri: CREATE_BUNDLE_RESOURCE_URI } },
		annotations: {
			readOnlyHint: false,
		...
		execute: async ({ context }) => {
			...
			const requests: ComponentRequest[] = context.components.map(
				(component) => ({
			...
			const strategy: PricingStrategy =
				context.fixedPrice != null
					? "fixed_price"
					: context.discountPercentage != null
						? "discount_percentage"
						: "sum";
			...
			const operation = await createProductBundle(
				credentials,
				title,
				plan.mutationComponents,
			);

			const { operation: finished } = await pollBundleOperation(
				credentials,
			...
			const variants = product.variants.nodes.map((variant) => ({
				...
				compareAtPrice:
					plan.pricing.savings > 0
						? plan.pricing.componentsTotal.toFixed(2)
						: null,
			}));
