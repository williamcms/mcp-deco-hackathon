...
function resolveChannel(order: ShopifyOrder): string {
  return order.channelInformation?.channelDefinition?.channelName ?? prettifySource(order.sourceName) ?? "Desconhecido";
}

function resolveRegion(order: ShopifyOrder): string {
  const address = order.shippingAddress ?? order.billingAddress;
  if (!address) return "Não informada";
  return address.provinceCode ?? address.province ?? address.countryCodeV2 ?? address.country ?? "Não informada";
}
...
function topN(entries: Array<[string, Bucket]>, limit: number): Array<[string, Bucket]> {
...
export interface AggregateOptions {
  ...
  /** Restricts the whole aggregation to line items whose product passes this check — e.g. bundle-only sales. */
...
export function aggregate(orders: ShopifyOrder[], options: AggregateOptions): CollectShopifySalesOutput {
  ...
  for (const order of orders) {
    ...
    for (const line of order.lineItems.nodes) {
      if (productFilter && !productFilter(line.product)) continue;
      ...
      const category = line.product?.category?.name ?? (line.product?.productType || null) ?? "Sem categoria";
      ...
      items.push({
        ...
        marginPct: lineMargin != null && paid > 0 ? round((lineMargin / paid) * 100) : null,
      });
    }
  }
  ...
  const categories = topN([...byCategory.entries()], 8).map(([name, bucket]) => ({
    name,
    revenue: round(bucket.revenue),
    units: bucket.units,
    marginPct: marginPct(bucket.revenueWithCost, bucket.cost),
