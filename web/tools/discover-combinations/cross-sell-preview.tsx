import { ArrowRightLeft, ChartNoAxesCombined, Package } from "lucide-react";
		...
		<span className="inline-flex items-center gap-1.5 bg-muted/60 px-2 py-1 rounded-md max-w-50 text-xs">
			{product.imageUrl ? (
				<img
					...
					alt=""
					className="rounded size-4 object-cover shrink-0"
				/>
			) : null}
 ...
 */
export function CrossSellPreview({
	result,
	busy,
	onPublish,
	onDismiss,
}: CrossSellPreviewProps) {
	...
	return (
		<div className="flex flex-col gap-4">
			<Card>
				<Row
					...
					title={
						isApplied
							? "Cross-sell gravado"
							: "Simulação — revise antes de gravar"
					}
				...
				<Row
					...
					right={
						<ProductPillList products={result.existingComplementaryProducts} />
					}
				/>
				<Row
				...
				<Row
					title="Lista final"
					right={
						<ProductPillList products={result.finalComplementaryProducts} />
					}
				/>
				{isApplied ? (
					<Row
				...
				{isApplied && result.campaign ? (
					<Row
						icon={<ChartNoAxesCombined className="size-4" />}
						title="Acompanhamento iniciado"
						description="Esta seleção foi registrada para comparar o período antes e depois da publicação."
					/>
				) : null}
			...
			{result.finalComplementaryProducts.length === 0 ? (
				<Empty>
					Selecione ao menos um produto de cross-sell no canvas antes de gerar.
				</Empty>
			) : null}

			<Alert icon={<ArrowRightLeft className="size-4" />}>
				Isso grava o metafield reservado de "produtos complementares" da Shopify
				— o mesmo que alimenta o card "Search & discovery" no admin e os widgets
				de recomendação de produto nos temas que leem esse campo. Não cria
				produto novo nem altera preço.
			</Alert>
				...
				{isApplied ? (
					...
					<>
						...
						<SmallButton
							active
							onClick={onPublish}
							disabled={busy || result.finalComplementaryProducts.length === 0}
						>
