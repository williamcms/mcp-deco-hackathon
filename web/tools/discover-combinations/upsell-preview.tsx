import { ArrowUpRight, ChartNoAxesCombined, Package } from "lucide-react";
		...
		<div className="flex flex-col gap-4">
			<Card>
				<Row
					...
					title={
						isApplied ? "Upsell gravado" : "Simulação — revise antes de gravar"
					}
				...
				<Row
					title="Lista final"
					right={<ProductPillList products={result.finalRelatedProducts} />}
				/>
				<Row
					...
					right={
						<span className="text-xs">
							{displayLabel(result.display.applied)}
						</span>
					}
				/>
				{isApplied ? (
			...
			{result.finalRelatedProducts.length === 0 ? (
				<Empty>Nenhum produto válido para recomendar como upgrade.</Empty>
			) : null}

			<Alert icon={<ArrowUpRight className="size-4" />}>
				Isso grava os produtos relacionados da Shopify — o bloco "você também
				pode gostar" na página do produto. Como a Shopify gera esses
				relacionados automaticamente, o modo de exibição é gravado junto; sem
				ele, os upgrades escolhidos aqui não apareceriam.
			</Alert>
				...
				{isApplied ? (
					...
					<>
						...
						<SmallButton
							active
							onClick={onPublish}
							disabled={busy || result.finalRelatedProducts.length === 0}
						>
